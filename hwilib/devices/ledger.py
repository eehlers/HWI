        ...
        for txin, psbt_in, i_num in zip(c_tx.vin, tx.inputs, range(len(c_tx.vin))):
            ...
            if psbt_in.non_witness_utxo:
                segwit_inputs.append(
                    {
                        "value": txin.prevout.serialize()
                        + struct.pack(
                            "<Q", psbt_in.non_witness_utxo.vout[txin.prevout.n].nValue
                        ),
                        "witness": True,
                        "sequence": seq_hex,
                    }
                ...
                segwit_inputs.append(
                    {
                        "value": txin.prevout.serialize()
                        + struct.pack("<Q", psbt_in.witness_utxo.nValue),
                        "witness": True,
                        "sequence": seq_hex,
                    }
                )
            ...
            if psbt_in.witness_utxo is not None and psbt_in.witness_utxo.is_p2sh():
                ...
                psbt_in.non_witness_utxo is not None
                and psbt_in.non_witness_utxo.vout[txin.prevout.n].is_p2sh()
            ):
                redeemscript = psbt_in.redeem_script
            ...
            if (
                len(witness_program) == 34
                and witness_program[0] == 0x00
                and witness_program[1] == 0x20
            ):
                ...
                if len(redeemscript) > 0:
                    ...
                    scriptCode = psbt_in.non_witness_utxo.vout[
                        txin.prevout.n
                    ].scriptPubKey
        ...
        if has_segwit:
            ...
            for i in range(len(segwit_inputs)):
                self.app.startUntrustedTransaction(
                    i == 0, i, segwit_inputs, blank_script_code, c_tx.nVersion
                )
            ...
            for i in range(len(segwit_inputs)):
                ...
                for signature_attempt in all_signature_attempts[i]:
                    ...
                    tx.inputs[i].partial_sigs[
                        ...
                        signature_attempt[0], auth, c_tx.nLockTime, 0x01
                    )
        elif has_legacy:
            first_input = True
            # Legacy signing if all inputs are legacy
